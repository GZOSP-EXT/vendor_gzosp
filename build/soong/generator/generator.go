...
func init() {
	android.RegisterModuleType("tesla_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = teslaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = teslaExpandVariables(ctx, dep_file)
	...
	cmd := teslaExpandVariables(ctx, String(g.properties.Cmd))
